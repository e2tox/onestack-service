...
module.exports = {
    ...
    format: function(request, errorCode, data) {
        ...
        if (accept.indexOf('xml') >= 0) {
            request.log(['ASG', 'SASG9101']);
            type = 'application/xml';
            payload = xml(request, error);
        }
        else if (accept.indexOf('text') >= 0) {
            request.log(['ASG', 'SASG9102']);
            type = 'text/html';
            payload = html(request, error);
        }
        else {
            request.log(['ASG', 'SASG9103']);
        ...
        if (error.statusCode === 401) {
            request.log(['ASG', 'SASG9104']);
            response = response.header('WWW-Authenticate', 'Basic realm="Asgard"');
        }
